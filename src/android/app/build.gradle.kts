...
val stageDebugSkillAssets by tasks.registering(Exec::class) {
    ...
    // Windows: bare "bash" resolves to the WSL stub in System32 (searched before
    // PATH by CreateProcess). Prefer Git Bash when present, fall back to "bash".
    val gitBash = listOf("D:/Git/bin/bash.exe", "C:/Program Files/Git/bin/bash.exe")
        .map(::File).firstOrNull { it.exists() }
    commandLine(gitBash?.absolutePath ?: "bash", script.absolutePath)
}
